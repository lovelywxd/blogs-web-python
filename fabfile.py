...
# 远程应用目录
_REMOTE_BASE_DIR = "/root/workspace/python-web"
def deploy():
    # 用时间来命名新版本
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo(" root:wxd www")
        sudo("chown -R root:wxd %s" % newdir)
...
def backup():
    ...
    dt = _now()
    f = "backup-webapp-%s.sql" % dt  # 创建一个包含时间的db文件名
    with cd("/tmp"):  # 远程计算机 "cd /tmp", 以下指令都在该目录下执行
        run("mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick webapp > %s" % (db_user, db_password, f)) 
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo("ln -s %s www" % old)
        sudo("chown wxd:wxd www")
        with settings(warn_only=True):  # 版本回退成功,重启服务器
            sudo("supervisor stop webapp")
            sudo("supervisor start webapp")
            sudo("/etc/init.d/nginx reload")
        print("ROLLBACKED OK")
...
def restore2local():
    ...
    sqls = [  # mysql语句列表
        "drop database if exists web;",  # 若本地已存在awesome数据库,删除之
        "create database web;",  # 新建数据库
        "grant select, insert, update, delete on web.* to '%s'@'localhost' identified by '%s';" % (db_user, db_password)
    ]
    for sql in sqls:
    ...
    local(r"mysql -u root -p%s web < backup/%s" % (p, restore_file[:-7]))  # 将选择的备份恢复到本地数据库
    with lcd(backup_dir):
